...
class RepoVis {
    ...
    clicked(event, p) {
        ...
        if (event.detail === 2 && p.data.is_directory) {
            this.currentRoot = p;
            this.zoomTo(p);
        }
    }
    ...
    zoomTo(p) {
        const root = this.svg.datum();
        
        // TODO: Implement smooth zoom transition
        // For now, just update center text
        this.centerText.text(p.data.name || 'root');
    }
